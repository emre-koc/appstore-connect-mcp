...
const directory = await mkdtemp(join(tmpdir(), "asc-mcp-dist-"));
...
const client = new Client({ name: "dist-smoke", version: "1" });
try {
  ...
  assert.equal(stderr, "");
  console.log(`compiled stdio smoke passed (${listed.tools.length} tools)`);
} finally {
  await client.close();
}
